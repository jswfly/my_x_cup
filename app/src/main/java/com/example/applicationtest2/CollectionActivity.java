...
public class CollectionActivity extends Activity implements AdapterView.OnItemClickListener, View.OnClickListener{
   ...
   @Override
    protected void onCreate(Bundle savedInstanceState) {
       ...
       adapter = new MainBodyAdapterCopy(CollectionActivity.this, R.layout.part_item_main_body,  mainList);
    ...
    private void initListDate() {
        ...
        MainList item = new MainList(R.mipmap.collection1, "清凉夏日常被着它总没错，相信我你不会吃亏！", "handsome, 今晚我有个礼物送你哦");
        mainList.add(item);
        item = new MainList(R.mipmap.collection2, "想为爸妈尽些孝心，就送这些实用的东西♪(^∇^*)", "今晚我有个礼物送你哦");
        mainList.add(item);
        item = new MainList(R.mipmap.collection3, "杯～杯～杯～， 杯子！！", "清新自然的绿色环境，会给人充满力量的正能量。夏季天气炎热");
        mainList.add(item);
        item = new MainList(R.mipmap.collection4, "就差你了！快进来", "handsome, 今晚我有个礼物送你哦");
        mainList.add(item);
        item = new MainList(R.mipmap.collection5, "二次元老司机带队啦！快上车！！！", "最近喜欢上的姑娘是个B站老司机、日本百事通、次元中毒患者，害怕有二次元壁？");
        mainList.add(item);
        item = new MainList(R.mipmap.collection6, "送给男票们千万不要错过，他会喜欢的", "你有一份礼物请查收！今晚我就是你的人了(✿◡‿◡)");
        mainList.add(item);
    }
